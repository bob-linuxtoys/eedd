...
HARDWARE
The bipolar stepper motor controller uses all four pins on a
BaseBoard connector to control the on/off state and direction
of two H-bridges connected to the two coils of a bipolar stepper
motor.  The four pins are assigned as coil A direction, coil A
on/off, coil B direction, and coil B on/off.  All outputs are
INVERTED to match the power-on state of the FPGA.  The D7HB
dual H-bridge card is often used with this peripheral.
